  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: "jarvis",
      content:
        "Hello! I'm Jarvis, your AI entertainment companion. I can help you discover new books and movies, track your progress, and provide personalized recommendations. What would you like to explore today?",
      timestamp: new Date(),
      suggestions: [
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.map((message) => (
          <div key={message.id} className={`flex gap-4 ${message.type === "user" ? "justify-end" : ""}`}>
            {message.type === "jarvis" && (
              <Avatar className="w-8 h-8 bg-gradient-to-br from-cyan-600 to-lime-500">
                <AvatarFallback>
            ...
            <div className={`max-w-2xl ${message.type === "user" ? "order-first" : ""}`}>
              <Card className={message.type === "user" ? "bg-cyan-600 text-white" : "bg-card"}>
            ...
            <Card className="bg-card">
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">Jarvis is thinking...</span>
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      <div className="border-t bg-card/50 backdrop-blur-sm p-6">
        <div className="flex gap-4">
          <div className="flex-1 relative">
            <Input
              placeholder="Ask Jarvis anything about books, movies, or your entertainment preferences..."
              value={inputValue}
